...
class PlayingCardView: UIView {
    ...
    @IBInspectable
    var rank: Int = 11 {didSet{setNeedsDisplay(); setNeedsLayout() }}//draw rect cand se schimba cartea}}
    @IBInspectable
    ...
    private func centeredAttributedString(_ string :String, fontSize: CGFloat)->NSAttributedString{
        ...
        font = UIFontMetrics(forTextStyle: .body).scaledFont(for: font)//daca setam din meniul tlf sa marim fontul
    ...
    private func createCornerLabel()-> UILabel{
        let label = UILabel()
        label.numberOfLines = 0// use as many as u need
        addSubview(label)
        return label
    }
    private func configureCornerLabel(_ label: UILabel){
        label.attributedText = cornerString
        label.frame.size = CGSize.zero//resetam masurile ca sa mearga sizetofit
        label.sizeToFit()
